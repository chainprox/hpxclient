...
def load_data_config_file(config_file):
    if not config_file:
        config_file = os.path.join(settings.CHAINPROX_DIR,
                                   settings.CHAINPROX_CONFIG_NAME)
...
def load_config():
    ...
    if "CHAINPROX_PUBLIC_KEY" in os.environ:
        print("Reading PUBLIC KEY from environment.")
        settings.PUBLIC_KEY = os.environ["CHAINPROX_PUBLIC_KEY"]

    if "CHAINPROX_SECRET_KEY" in os.environ:
        print("Reading SECRET KEY from environment.")
        settings.SECRET_KEY = os.environ["CHAINPROX_SECRET_KEY"]
